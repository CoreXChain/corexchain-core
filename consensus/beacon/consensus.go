...
// https://corex.internal/EIPs/blob/master/EIPS/eip-3675.md
//
// The beacon here is a half-functional consensus engine with partial functions which
// is only used for necessary consensus checks. The legacy consensus engine can be any
// engine implements the consensus interface (except the beacon itself).
type Beacon struct {
	ethone consensus.Engine // Original consensus engine used in eth1, e.g. corexhash or clique
}
...
// stock CoreXChain consensus engine.
...
// rules of the CoreXChain consensus engine.
...
// verifyHeader checks whether a header conforms to the consensus rules of the
// stock CoreXChain consensus engine. The difference between the beacon and classic is
...
func (beacon *Beacon) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, withdrawals []*types.Withdrawal) {
	...
	for _, w := range withdrawals {
		// Convert amount from gcorex to nano.
