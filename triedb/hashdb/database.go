...
type Database struct {
	diskdb   corexdb.Database // Persistent storage for matured trie nodes
...
func New(diskdb corexdb.Database, config *Config, resolver ChildResolver) *Database {
	if config == nil {
		config = Defaults
	}
...
// are referenced tocorexer by database itself.
...
func (db *Database) commit(hash common.Hash, batch corexdb.Batch, uncacher *cleaner) error {
	// If the node does not exist, it's a previously committed node
	node, ok := db.dirties[hash]
	if !ok {
		return nil
