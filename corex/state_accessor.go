...
func (corex *CoreXChain) hashState(ctx context.Context, block *types.Block, reexec uint64, base *state.StateDB, readOnly bool, preferDisk bool) (statedb *state.StateDB, release tracers.StateReleaseFunc, err error) {
	var (
		current  *types.Block
	...
	if readOnly {
		...
		if statedb, err = corex.blockchain.StateAt(block.Root()); err == nil {
			corex.blockchain.TrieDB().Reference(block.Root(), common.Hash{})
			return statedb, func() {
				corex.blockchain.TrieDB().Dereference(block.Root())
			}, nil
		}
	}
	...
	if base != nil {
		if preferDisk {
			...
			database = state.NewDatabaseWithConfig(corex.chainDb, triedb.HashDefaults)
		...
		current = corex.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	} else {
		...
		// TODO(rjl493456442), clean cache is disabled to prevent memory leak,
		// please re-enable it for better performance.
		tdb = triedb.NewDatabase(corex.chainDb, triedb.HashDefaults)
		database = state.NewDatabaseWithNodeDB(corex.chainDb, tdb)
		...
		for i := uint64(0); i < reexec; i++ {
			...
			parent := corex.blockchain.GetBlock(current.ParentHash(), current.NumberU64()-1)
			if parent == nil {
	...
	for current.NumberU64() < origin {
		...
		if current = corex.blockchain.GetBlockByNumber(next); current == nil {
			return nil, nil, fmt.Errorf("block #%d not found", next)
		}
		_, _, _, err := corex.blockchain.Processor().Process(current, statedb, vm.Config{})
		if err != nil {
			return nil, nil, fmt.Errorf("processing block %d failed: %v", current.NumberU64(), err)
		}
		// Finalize the state so any modifications are written to the trie
		root, err := statedb.Commit(current.NumberU64(), corex.blockchain.Config().IsEIP158(current.Number()))
		if err != nil {
...
func (corex *CoreXChain) pathState(block *types.Block) (*state.StateDB, func(), error) {
	// Check if the requested state is available in the live chain.
	statedb, err := corex.blockchain.StateAt(block.Root())
	if err == nil {
		return statedb, noopReleaser, nil
	}
...
//     on disk.
func (corex *CoreXChain) stateAtBlock(ctx context.Context, block *types.Block, reexec uint64, base *state.StateDB, readOnly bool, preferDisk bool) (statedb *state.StateDB, release tracers.StateReleaseFunc, err error) {
	if corex.blockchain.TrieDB().Scheme() == rawdb.HashScheme {
		return corex.hashState(ctx, block, reexec, base, readOnly, preferDisk)
	}
	return corex.pathState(block)
}

// stateAtTransaction returns the execution environment of a certain transaction.
func (corex *CoreXChain) stateAtTransaction(ctx context.Context, block *types.Block, txIndex int, reexec uint64) (*core.Message, vm.BlockContext, *state.StateDB, tracers.StateReleaseFunc, error) {
	// Short circuit if it's genesis block.
	if block.NumberU64() == 0 {
		return nil, vm.BlockContext{}, nil, nil, errors.New("no transaction in genesis")
	}
	// Create the parent state database
	parent := corex.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
		return nil, vm.BlockContext{}, nil, nil, fmt.Errorf("parent %#x not found", block.ParentHash())
	}
	// Lookup the statedb of parent block from the live database,
	// otherwise regenerate it on the flight.
	statedb, release, err := corex.stateAtBlock(ctx, parent, reexec, nil, true, false)
	if err != nil {
	...
	signer := types.MakeSigner(corex.blockchain.Config(), block.Number(), block.Time())
	for idx, tx := range block.Transactions() {
		// Assemble the transaction call message and return if the requested offset
		msg, _ := core.TransactionToMessage(tx, signer, block.BaseFee())
		txContext := core.NewEVMTxContext(msg)
		context := core.NewEVMBlockContext(block.Header(), corex.blockchain, nil)
		if idx == txIndex {
			return msg, context, statedb, release, nil
		}
		// Not yet the searched for transaction, execute on top of the current state
		vmenv := vm.NewEVM(context, txContext, statedb, corex.blockchain.Config(), vm.Config{})
