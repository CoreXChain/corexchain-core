...
// DebugAPI is the collection of CoreXChain full node APIs for debugging the
// protocol.
...
func NewDebugAPI(corex *CoreXChain) *DebugAPI {
	return &DebugAPI{corex: corex}
}
...
func (api *DebugAPI) DumpBlock(blockNr rpc.BlockNumber) (state.Dump, error) {
	...
	if blockNr == rpc.PendingBlockNumber {
		...
		// the block producer and operate on those
		_, stateDb := api.corex.production.Pending()
		if stateDb == nil {
	...
	case rpc.LatestBlockNumber:
		header = api.corex.blockchain.CurrentBlock()
	case rpc.FinalizedBlockNumber:
		header = api.corex.blockchain.CurrentFinalBlock()
	case rpc.SafeBlockNumber:
		header = api.corex.blockchain.CurrentSafeBlock()
	default:
		block := api.corex.blockchain.GetBlockByNumber(uint64(blockNr))
		if block == nil {
	...
	stateDb, err := api.corex.BlockChain().StateAt(header.Root)
	if err != nil {
...
func (api *DebugAPI) Preimage(ctx context.Context, hash common.Hash) (hexutil.Bytes, error) {
	if preimage := rawdb.ReadPreimage(api.corex.ChainDb(), hash); preimage != nil {
		return preimage, nil
	}
...
func (api *DebugAPI) GetBadBlocks(ctx context.Context) ([]*BadBlockArgs, error) {
	var (
		blocks  = rawdb.ReadAllBadBlocks(api.corex.chainDb)
	...
	for _, block := range blocks {
		...
		blockJSON = corexapi.RPCMarshalBlock(block, true, true, api.corex.APIBackend.ChainConfig())
	...
	if number, ok := blockNrOrHash.Number(); ok {
		if number == rpc.PendingBlockNumber {
			...
			case rpc.SafeBlockNumber:
				header = api.corex.blockchain.CurrentSafeBlock()
			default:
				block := api.corex.blockchain.GetBlockByNumber(uint64(number))
				if block == nil {
		...
		stateDb, err = api.corex.BlockChain().StateAt(block.Root())
		if err != nil {
			return state.Dump{}, err
		}
	} else {
...
func (api *DebugAPI) StorageRangeAt(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash, txIndex int, contractAddress common.Address, keyStart hexutil.Bytes, maxResult int) (StorageRangeResult, error) {
	...
	block, err := api.corex.APIBackend.BlockByNumberOrHash(ctx, blockNrOrHash)
	if err != nil {
	...
	_, _, statedb, release, err := api.corex.stateAtTransaction(ctx, block, txIndex, 0)
	if err != nil {
		return StorageRangeResult{}, err
	}
	defer release()
...
func (api *DebugAPI) GetModifiedAccountsByNumber(startNum uint64, endNum *uint64) ([]common.Address, error) {
	...
	startBlock = api.corex.blockchain.GetBlockByNumber(startNum)
	if startBlock == nil {
	...
	if endNum == nil {
		endBlock = startBlock
		startBlock = api.corex.blockchain.GetBlockByHash(startBlock.ParentHash())
		if startBlock == nil {
			return nil, fmt.Errorf("block %x has no parent", endBlock.Number())
		}
	} else {
		endBlock = api.corex.blockchain.GetBlockByNumber(*endNum)
		if endBlock == nil {
...
func (api *DebugAPI) GetModifiedAccountsByHash(startHash common.Hash, endHash *common.Hash) ([]common.Address, error) {
	var startBlock, endBlock *types.Block
	startBlock = api.corex.blockchain.GetBlockByHash(startHash)
	if startBlock == nil {
	...
	if endHash == nil {
		...
		endBlock = api.corex.blockchain.GetBlockByHash(*endHash)
		if endBlock == nil {
...
func (api *DebugAPI) getModifiedAccounts(startBlock, endBlock *types.Block) ([]common.Address, error) {
	...
	triedb := api.corex.BlockChain().TrieDB()
...
func (api *DebugAPI) GetAccessibleState(from, to rpc.BlockNumber) (uint64, error) {
	...
	var resolveNum = func(num rpc.BlockNumber) (uint64, error) {
		...
		if num.Int64() < 0 {
			block := api.corex.blockchain.CurrentBlock()
			if block == nil {
	...
	for i := int64(start); i != int64(end); i += delta {
		...
		h := api.corex.BlockChain().GetHeaderByNumber(uint64(i))
		if h == nil {
			return 0, fmt.Errorf("missing header %d", i)
		}
		if ok, _ := api.corex.ChainDb().Has(h.Root[:]); ok {
			return uint64(i), nil
		}
	}
...
func (api *DebugAPI) SetTrieFlushInterval(interval string) error {
	...
	api.corex.blockchain.SetTrieFlushInterval(t)
	return nil
}
...
func (api *DebugAPI) GetTrieFlushInterval() (string, error) {
	...
	return api.corex.blockchain.GetTrieFlushInterval().String(), nil
}
