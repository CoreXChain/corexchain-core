...
const (
	...
	// retrieving a maximum of one bundle transaction at a time to minimize hogging
...
var (
	txAnnounceInMeter          = metrics.NewRegisteredMeter("corex/fetcher/transaction/announces/in", nil)
	txAnnounceKnownMeter       = metrics.NewRegisteredMeter("corex/fetcher/transaction/announces/known", nil)
	txAnnounceUnderpricedMeter = metrics.NewRegisteredMeter("corex/fetcher/transaction/announces/underpriced", nil)
	txAnnounceDOSMeter         = metrics.NewRegisteredMeter("corex/fetcher/transaction/announces/dos", nil)

	txBroadcastInMeter          = metrics.NewRegisteredMeter("corex/fetcher/transaction/broadcasts/in", nil)
	txBroadcastKnownMeter       = metrics.NewRegisteredMeter("corex/fetcher/transaction/broadcasts/known", nil)
	txBroadcastUnderpricedMeter = metrics.NewRegisteredMeter("corex/fetcher/transaction/broadcasts/underpriced", nil)
	txBroadcastOtherRejectMeter = metrics.NewRegisteredMeter("corex/fetcher/transaction/broadcasts/otherreject", nil)

	txRequestOutMeter     = metrics.NewRegisteredMeter("corex/fetcher/transaction/request/out", nil)
	txRequestFailMeter    = metrics.NewRegisteredMeter("corex/fetcher/transaction/request/fail", nil)
	txRequestDoneMeter    = metrics.NewRegisteredMeter("corex/fetcher/transaction/request/done", nil)
	txRequestTimeoutMeter = metrics.NewRegisteredMeter("corex/fetcher/transaction/request/timeout", nil)

	txReplyInMeter          = metrics.NewRegisteredMeter("corex/fetcher/transaction/replies/in", nil)
	txReplyKnownMeter       = metrics.NewRegisteredMeter("corex/fetcher/transaction/replies/known", nil)
	txReplyUnderpricedMeter = metrics.NewRegisteredMeter("corex/fetcher/transaction/replies/underpriced", nil)
	txReplyOtherRejectMeter = metrics.NewRegisteredMeter("corex/fetcher/transaction/replies/otherreject", nil)

	txFetcherWaitingPeers   = metrics.NewRegisteredGauge("corex/fetcher/transaction/waiting/peers", nil)
	txFetcherWaitingHashes  = metrics.NewRegisteredGauge("corex/fetcher/transaction/waiting/hashes", nil)
	txFetcherQueueingPeers  = metrics.NewRegisteredGauge("corex/fetcher/transaction/queueing/peers", nil)
	txFetcherQueueingHashes = metrics.NewRegisteredGauge("corex/fetcher/transaction/queueing/hashes", nil)
	txFetcherFetchingPeers  = metrics.NewRegisteredGauge("corex/fetcher/transaction/fetching/peers", nil)
	txFetcherFetchingHashes = metrics.NewRegisteredGauge("corex/fetcher/transaction/fetching/hashes", nil)
)
...
type txAnnounce struct {
	...
	metas  []*txMetadata // Batch of metadatas associated with the hashes (nil before corex/68)
}
...
type TxFetcher struct {
	...
	// Callbacks
	hasTx    func(common.Hash) bool             // Retrieves a tx from the local flowspace
	addTxs   func([]*types.Transaction) []error // Insert a batch of transactions into local flowspace
...
// Enqueue imports a batch of received transaction into the flow space
...
func (f *TxFetcher) Enqueue(peer string, txs []*types.Transaction, direct bool) error {
	...
	for i := 0; i < len(txs); i += 128 {
		...
		for j, err := range f.addTxs(batch) {
			...
			// announcement.
			if errors.Is(err, flowspace.ErrUnderpriced) || errors.Is(err, flowspace.ErrReplaceUnderpriced) {
			...
			case errors.Is(err, flowspace.ErrAlreadyKnown):
				duplicate++

			case errors.Is(err, flowspace.ErrUnderpriced) || errors.Is(err, flowspace.ErrReplaceUnderpriced):
				underpriced++
...
func (f *TxFetcher) rescheduleTimeout(timer *mclock.Timer, trigger chan struct{}) {
	...
	for _, req := range f.requests {
		// If this request already timed out, skip it altocorexer
		if req.hashes == nil {
			continue
		}
...
func (f *TxFetcher) scheduleFetches(timer *mclock.Timer, timeout chan struct{}, whitelist map[string]struct{}) {
	...
	f.forEachPeer(actives, func(peer string) {
		...
		f.forEachAnnounce(f.announces[peer], func(hash common.Hash, meta *txMetadata) bool {
			...
			if meta != nil { // Only set corex/68 and upwards
