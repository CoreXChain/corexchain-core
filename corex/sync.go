...
func (h *handler) syncTransactions(p *corex.Peer) {
	var hashes []common.Hash
	for _, batch := range h.flowspace.Pending(flowspace.PendingFilter{OnlyPlainTxs: true}) {
...
func (cs *chainSyncer) nextSyncOp() *chainSyncOp {
	...
	// If a beacon client once took over control, disable the entire classic sync
...
func peerToSyncOp(mode downloader.SyncMode, p *corex.Peer) *chainSyncOp {
...
func (h *handler) doSync(op *chainSyncOp) error {
	...
	if head.Number.Uint64() > 0 {
		...
		// degenerate connectivity, but it should be healthy for the corenet too to
