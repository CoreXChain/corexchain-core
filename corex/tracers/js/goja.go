...
import (
	...
	jsassets "com.corexkey/corex/tracers/js/internal/tracers"
	"com.corexkey/crypto"
)

var assetTracers = make(map[string]string)

// init retrieves the JavaScript transaction tracers included in corex-chain.
func init() {
	var err error
...
// `enter` and `exit` always go tocorexer.
...
func (t *jsTracer) CaptureStart(env *vm.EVM, from common.Address, to common.Address, create bool, input []byte, gas uint64, value *big.Int) {
	...
	valueFlowFeeBig, err := t.toBig(t.vm, env.TxContext.ValueFlowFee.String())
	if err != nil {
		cancel(err)
		return
	}
	t.ctx["valueFlowFee"] = valueFlowFeeBig
