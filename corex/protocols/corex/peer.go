...
// Peer is a collection of relevant information we have about a `corex` peer.
type Peer struct {
	...
	*p2p.Peer                   // The embedded P2P package peer
	...
	flowspace   FlowSpace          // Transaction pool used by the broadcasters for liveness checks
...
// version.
func NewPeer(version uint, p *p2p.Peer, rw p2p.MsgReadWriter, flowspace FlowSpace) *Peer {
	peer := &Peer{
		id:              p.ID().String(),
		Peer:            p,
		rw:              rw,
...
// Version retrieves the peer's negotiated `corex` protocol version.
