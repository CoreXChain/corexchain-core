...
type accountTask struct {
	...
	// this additional field will be ignored by legacy CoreX. The only side
	...
	genBatch corexdb.Batch // Batch used by the node generator
...
// Syncer is a CoreXChain account and storage trie syncer based on snapshots and
...
type Syncer struct {
	db     corexdb.KeyValueStore // Database to store the trie nodes into (and dedup)
	...
	stateWriter        corexdb.Batch      // Shared batch writer used for persisting raw states
...
// NewSyncer creates a new snapshot syncer to download the CoreXChain state over the
// snap protocol.
func NewSyncer(db corexdb.KeyValueStore, scheme string) *Syncer {
	return &Syncer{
		db:     db,
		scheme: scheme,
	...
	if status := rawdb.ReadSnapshotSyncStatus(s.db); status != nil {
		if err := json.Unmarshal(status, &progress); err != nil {
			...
			for _, task := range s.tasks {
				...
				task.genBatch = corexdb.HookedBatch{
					Batch: s.db.NewBatch(),
				...
				for accountHash, subtasks := range task.SubTasks {
					for _, subtask := range subtasks {
						...
						subtask.genBatch = corexdb.HookedBatch{
							Batch: s.db.NewBatch(),
							OnPut: func(key []byte, value []byte) {
...
func (s *Syncer) assignTrienodeHealTasks(success chan *trienodeHealResponse, fail chan *trienodeHealRequest, cancel chan struct{}) {
	...
	for len(s.healer.trieTasks) > 0 || s.healer.scheduler.Pending() > 0 {
		...
		// tocorexer with bytecodes, so we need to queue them combined.
		var (
...
func (s *Syncer) assignBytecodeHealTasks(success chan *bytecodeHealResponse, fail chan *bytecodeHealRequest, cancel chan struct{}) {
	...
	for len(s.healer.codeTasks) > 0 || s.healer.scheduler.Pending() > 0 {
		// If there are not enough trie tasks queued to fully assign, fill the
		// queue from the state sync scheduler. The trie synced schedules these
		// tocorexer with trie nodes, so we need to queue them combined.
		var (
			have = len(s.healer.trieTasks) + len(s.healer.codeTasks)
			want = maxTrieRequestCount + maxCodeRequestCount
		)
...
func (s *Syncer) processAccountResponse(res *accountResponse) {
	...
	// no longer possible in corexchain as self-destruction is disabled in Cancun
	...
	if len(res.hashes) > 0 {
		...
		for hash := range res.task.SubTasks {
			...
			// TODO(rjl493456442) degrade the log level before merging.
			// It should never happen in corexchain.
...
func (s *Syncer) commitHealer(force bool) {
	if !force && s.healer.scheduler.MemSize() < corexdb.IdealBatchSize {
		return
	}
	batch := s.db.NewBatch()
...
func (s *Syncer) onHealState(paths [][]byte, value []byte) error {
	...
	if s.stateWriter.ValueSize() > corexdb.IdealBatchSize {
