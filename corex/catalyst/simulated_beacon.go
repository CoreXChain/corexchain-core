...
func NewSimulatedBeacon(period uint64, corex *corex.CoreXChain) (*SimulatedBeacon, error) {
	block := corex.BlockChain().CurrentBlock()
	current := engine.ForkchoiceStateV1{
		HeadBlockHash:      block.Hash(),
		SafeBlockHash:      block.Hash(),
		FinalizedBlockHash: block.Hash(),
	}
	engineAPI := newConsensusAPIWithoutHeartbeat(corex)
...
func (c *SimulatedBeacon) sealBlock(withdrawals []*types.Withdrawal, timestamp uint64) error {
	...
	if header := c.corex.BlockChain().CurrentBlock(); c.curForkchoiceState.HeadBlockHash != header.Hash() {
...
func (c *SimulatedBeacon) finalizedBlockHash(number uint64) *common.Hash {
	...
	if finalizedBlock := c.corex.BlockChain().GetBlockByNumber(finalizedNumber); finalizedBlock != nil {
...
func (c *SimulatedBeacon) Commit() common.Hash {
	...
	return c.corex.BlockChain().CurrentBlock().Hash()
}

// Rollback un-sends previously added transactions.
func (c *SimulatedBeacon) Rollback() {
	// Flush all transactions from the flow spaces
	maxUint256 := new(big.Int).Sub(new(big.Int).Lsh(common.Big1, 256), common.Big1)
	c.corex.FlowSpace().SetGasTip(maxUint256)
	// Set the priority fee back to accept new transactions
	// TODO (Marius van der Wijden): set priority fee to parameter passed by config
	c.corex.FlowSpace().SetGasTip(big.NewInt(params.GWei))
}

// Fork sets the head to the provided hash.
func (c *SimulatedBeacon) Fork(parentHash common.Hash) error {
	if len(c.corex.FlowSpace().Pending(flowspace.PendingFilter{})) != 0 {
		return errors.New("pending block dirty")
	}
	parent := c.corex.BlockChain().GetBlockByHash(parentHash)
	if parent == nil {
		return errors.New("parent not found")
	}
	return c.corex.BlockChain().SetHead(parent.NumberU64())
}
...
func (c *SimulatedBeacon) AdjustTime(adjustment time.Duration) error {
	...
	parent := c.corex.BlockChain().CurrentBlock()
	if parent == nil {
		return errors.New("parent not found")
	}
	withdrawals := c.withdrawals.gatherPending(10)
