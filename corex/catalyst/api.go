...
func Register(stack *node.Node, backend *corex.CoreXChain) error {
	log.Warn("Engine API enabled", "protocol", "corex")
...
type ConsensusAPI struct {
	...
	//     persist any bad block information to disk as a bug in CoreX could end
	//     up blocking a valid chain, even if a later CoreX update would accept
	//     it.
	//   - Bad blocks will get forgotten after a certain threshold of import
	//     attempts and will be retried. The rationale is that if the network
	//     really-really-really tries to feed us a block, we should give it a
	//     new chance, perhaps us being racey instead of the block being legit
	//     bad (this happened in CoreX at a point with import vs. pending race).
	...
	// CoreX can appear to be stuck or do strange things if the beacon client is
...
func NewConsensusAPI(corex *corex.CoreXChain) *ConsensusAPI {
	api := newConsensusAPIWithoutHeartbeat(corex)
	go api.heartbeat()
	return api
}

// newConsensusAPIWithoutHeartbeat creates a new consensus api for the SimulatedBeacon Node.
func newConsensusAPIWithoutHeartbeat(corex *corex.CoreXChain) *ConsensusAPI {
	if corex.BlockChain().Config().TerminalTotalDifficulty == nil {
	...
	corex.Downloader().SetBadBlockCallback(api.setInvalidAncestor)
	return api
}
...
func (api *ConsensusAPI) ForkchoiceUpdatedV1(update engine.ForkchoiceStateV1, payloadAttributes *engine.PayloadAttributes) (engine.ForkChoiceResponse, error) {
	if payloadAttributes != nil {
		...
		if api.corex.BlockChain().Config().IsShanghai(api.corex.BlockChain().Config().LondonBlock, payloadAttributes.Timestamp) {
...
func (api *ConsensusAPI) ForkchoiceUpdatedV2(update engine.ForkchoiceStateV1, params *engine.PayloadAttributes) (engine.ForkChoiceResponse, error) {
	if params != nil {
		switch api.corex.BlockChain().Config().LatestFork(params.Timestamp) {
		case forks.Paris:
...
func (api *ConsensusAPI) ForkchoiceUpdatedV3(update engine.ForkchoiceStateV1, params *engine.PayloadAttributes) (engine.ForkChoiceResponse, error) {
	if params != nil {
		// TODO(matt): according to https://corex.internal/execution-apis/pull/498,
		...
		if api.corex.BlockChain().Config().LatestFork(params.Timestamp) != forks.Cancun {
...
func (api *ConsensusAPI) forkchoiceUpdated(update engine.ForkchoiceStateV1, payloadAttributes *engine.PayloadAttributes, payloadVersion engine.PayloadVersion, simulatorMode bool) (engine.ForkChoiceResponse, error) {
	...
	// reason.
	block := api.corex.BlockChain().GetBlockByHash(update.HeadBlockHash)
	if block == nil {
		...
		// the future to resolve from the `corex` network, but it's an unexpected case
		...
		// Before we do however, make sure any classic sync in switched off so we
		// don't accidentally have 2 cycles running.
		if merger := api.corex.Merger(); !merger.TDDReached() {
			merger.ReachTTD()
			api.corex.Downloader().Cancel()
		}
		...
		if err := api.corex.Downloader().BeaconSync(api.corex.SyncMode(), header, finalized); err != nil {
	...
	if block.Difficulty().BitLen() > 0 || block.NumberU64() == 0 {
		var (
			td  = api.corex.BlockChain().GetTd(update.HeadBlockHash, block.NumberU64())
			ptd = api.corex.BlockChain().GetTd(block.ParentHash(), block.NumberU64()-1)
			ttd = api.corex.BlockChain().Config().TerminalTotalDifficulty
		)
	...
	if rawdb.ReadCanonicalHash(api.corex.ChainDb(), block.NumberU64()) != update.HeadBlockHash {
		// Block is not canonical, set head.
		if latestValid, err := api.corex.BlockChain().SetCanonical(block); err != nil {
			return engine.ForkChoiceResponse{PayloadStatus: engine.PayloadStatusV1{Status: engine.INVALID, LatestValidHash: &latestValid}}, err
		}
	} else if api.corex.BlockChain().CurrentBlock().Hash() == update.HeadBlockHash {
		...
		log.Info("Ignoring beacon update to old head", "number", block.NumberU64(), "hash", update.HeadBlockHash, "age", common.PrettyAge(time.Unix(int64(block.Time()), 0)), "have", api.corex.BlockChain().CurrentBlock().Number)
	...
	if update.FinalizedBlockHash != (common.Hash{}) {
		if merger := api.corex.Merger(); !merger.PoSFinalized() {
			merger.FinalizePoS()
		}
		// If the finalized block is not in our canonical tree, something is wrong
		finalBlock := api.corex.BlockChain().GetBlockByHash(update.FinalizedBlockHash)
		if finalBlock == nil {
			log.Warn("Final block not available in database", "hash", update.FinalizedBlockHash)
			return engine.STATUS_INVALID, engine.InvalidForkChoiceState.With(errors.New("final block not available in database"))
		} else if rawdb.ReadCanonicalHash(api.corex.ChainDb(), finalBlock.NumberU64()) != update.FinalizedBlockHash {
			log.Warn("Final block not in canonical chain", "number", block.NumberU64(), "hash", update.HeadBlockHash)
			return engine.STATUS_INVALID, engine.InvalidForkChoiceState.With(errors.New("final block not in canonical chain"))
		}
		// Set the finalized block
		api.corex.BlockChain().SetFinalized(finalBlock.Header())
	}
	// Check if the safe block hash is in our canonical tree, if not something is wrong
	if update.SafeBlockHash != (common.Hash{}) {
		safeBlock := api.corex.BlockChain().GetBlockByHash(update.SafeBlockHash)
		if safeBlock == nil {
			log.Warn("Safe block not available in database")
			return engine.STATUS_INVALID, engine.InvalidForkChoiceState.With(errors.New("safe block not available in database"))
		}
		if rawdb.ReadCanonicalHash(api.corex.ChainDb(), safeBlock.NumberU64()) != update.SafeBlockHash {
			log.Warn("Safe block not in canonical chain")
			return engine.STATUS_INVALID, engine.InvalidForkChoiceState.With(errors.New("safe block not in canonical chain"))
		}
		// Set the safe block
		api.corex.BlockChain().SetSafe(safeBlock.Header())
	}
	// If payload generation was requested, create a new block to be potentially
	// sealed by the beacon client. The payload will be requested later, and we
	// will replace it arbitrarily many times in between.
	if payloadAttributes != nil {
		args := &production.BuildPayloadArgs{
		...
		// the flow space running its internal reset operation on a back-
		...
		if simulatorMode {
			if err := api.corex.FlowSpace().Sync(); err != nil {
				log.Error("Failed to sync flow space", "err", err)
				return valid(nil), engine.InvalidPayloadAttributes.With(err)
			}
		}
		payload, err := api.corex.Producer().BuildPayload(args)
		if err != nil {
...
func (api *ConsensusAPI) ExchangeTransitionConfigurationV1(config engine.TransitionConfigurationV1) (*engine.TransitionConfigurationV1, error) {
	...
	if ttd == nil || ttd.Cmp(config.TerminalTotalDifficulty.ToInt()) != 0 {
		log.Warn("Invalid TTD configured", "corex", ttd, "beacon", config.TerminalTotalDifficulty)
		return nil, fmt.Errorf("invalid ttd: execution %v consensus %v", ttd, config.TerminalTotalDifficulty)
	}
	if config.TerminalBlockHash != (common.Hash{}) {
		if hash := api.corex.BlockChain().GetCanonicalHash(uint64(config.TerminalBlockNumber)); hash == config.TerminalBlockHash {
...
func (api *ConsensusAPI) NewPayloadV2(params engine.ExecutableData) (engine.PayloadStatusV1, error) {
	if api.corex.BlockChain().Config().IsCancun(api.corex.BlockChain().Config().LondonBlock, params.Timestamp) {
	...
	if api.corex.BlockChain().Config().LatestFork(params.Timestamp) == forks.Shanghai {
		if params.Withdrawals == nil {
	...
	if params.ExcessBundleResource != nil {
		return engine.PayloadStatusV1{Status: engine.INVALID}, engine.InvalidParams.With(errors.New("non-nil excessBundleResource pre-cancun"))
	}
	if params.BundleResourceUsed != nil {
		return engine.PayloadStatusV1{Status: engine.INVALID}, engine.InvalidParams.With(errors.New("non-nil bundleResourceUsed pre-cancun"))
	}
	return api.newPayload(params, nil, nil)
}
...
func (api *ConsensusAPI) NewPayloadV3(params engine.ExecutableData, versionedHashes []common.Hash, beaconRoot *common.Hash) (engine.PayloadStatusV1, error) {
	...
	if params.ExcessBundleResource == nil {
		return engine.PayloadStatusV1{Status: engine.INVALID}, engine.InvalidParams.With(errors.New("nil excessBundleResource post-cancun"))
	}
	if params.BundleResourceUsed == nil {
		return engine.PayloadStatusV1{Status: engine.INVALID}, engine.InvalidParams.With(errors.New("nil bundleResourceUsed post-cancun"))
	}
...
func (api *ConsensusAPI) newPayload(params engine.ExecutableData, versionedHashes []common.Hash, beaconRoot *common.Hash) (engine.PayloadStatusV1, error) {
	...
	//      api.corex.BlockChain().InsertBlockWithoutSetHead, where it is blocked on
	...
	if block := api.corex.BlockChain().GetBlockByHash(params.BlockHash); block != nil {
	...
	parent := api.corex.BlockChain().GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
	...
	var (
		ptd  = api.corex.BlockChain().GetTd(parent.Hash(), parent.NumberU64())
		ttd  = api.corex.BlockChain().Config().TerminalTotalDifficulty
		gptd = api.corex.BlockChain().GetTd(parent.ParentHash(), parent.NumberU64()-1)
	)
	if ptd.Cmp(ttd) < 0 {
	...
	if api.corex.SyncMode() != downloader.FullSync {
		return api.delayPayloadImport(block)
	}
	if !api.corex.BlockChain().HasBlockAndState(block.ParentHash(), block.NumberU64()-1) {
		api.remoteBlocks.put(block.Hash(), block.Header())
		log.Warn("State not available, ignoring new payload")
		return engine.PayloadStatusV1{Status: engine.ACCEPTED}, nil
	}
	log.Trace("Inserting block without sethead", "hash", block.Hash(), "number", block.Number)
	if err := api.corex.BlockChain().InsertBlockWithoutSetHead(block); err != nil {
...
func (api *ConsensusAPI) delayPayloadImport(block *types.Block) (engine.PayloadStatusV1, error) {
	...
	err := api.corex.Downloader().BeaconExtend(api.corex.SyncMode(), block.Header())
	if err == nil {
...
func (api *ConsensusAPI) checkInvalidAncestor(check common.Hash, head common.Hash) *engine.PayloadStatusV1 {
	...
	if header := api.corex.BlockChain().GetHeader(invalid.ParentHash, invalid.Number.Uint64()-1); header != nil && header.Difficulty.Sign() != 0 {
...
func (api *ConsensusAPI) heartbeat() {
	...
	if api.corex.BlockChain().Config().TerminalTotalDifficulty == nil {
		return
	}
	...
	for {
		...
		if api.corex.BlockChain().Config().TerminalTotalDifficultyPassed || api.corex.Merger().TDDReached() {
...
func (api *ConsensusAPI) GetPayloadBodiesByHashV1(hashes []common.Hash) []*engine.ExecutionPayloadBodyV1 {
	...
	for i, hash := range hashes {
		block := api.corex.BlockChain().GetBlockByHash(hash)
...
func (api *ConsensusAPI) GetPayloadBodiesByRangeV1(start, count hexutil.Uint64) ([]*engine.ExecutionPayloadBodyV1, error) {
	...
	current := api.corex.BlockChain().CurrentBlock().Number.Uint64()
	...
	for i := uint64(start); i <= last; i++ {
		block := api.corex.BlockChain().GetBlockByNumber(i)
