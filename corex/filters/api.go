...
// information related to the CoreXChain protocol such as blocks, transactions and logs.
...
// `corex_getFilterChanges` polling method that is also used for log filters.
...
// transaction enters the flow space. If fullTx is true the full tx is
			...
			case txs := <-txs:
				...
				for _, tx := range txs {
					if fullTx != nil && *fullTx {
						rpcTx := corexapi.NewRPCPendingTransaction(tx, latest, chainConfig)
...
// It is part of the filter package since polling goes with corex_getFilterChanges.
...
func (api *FilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(corexchain.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
// Same as corexchain.FilterQuery but with UnmarshalJSON() method.
type FilterCriteria corexchain.FilterQuery
...
func (api *FilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	...
	logsSub, err := api.events.SubscribeLogs(corexchain.FilterQuery(crit), logs)
	if err != nil {
		return "", err
	}
...
func (api *FilterAPI) GetFilterChanges(id rpc.ID) (interface{}, error) {
	...
	if f, found := api.filters[id]; found {
		...
		case PendingTransactionsSubscription:
			if f.fullTx {
				txs := make([]*corexapi.RPCTransaction, 0, len(f.txs))
				for _, tx := range f.txs {
					txs = append(txs, corexapi.NewRPCPendingTransaction(tx, latest, chainConfig))
				}
				f.txs = nil
				return txs, nil
			} else {
