...
// Package filters implements an corexchain filtering system for block,
...
type subscription struct {
	...
	typ       Type
	created   time.Time
	logsCrit  corexchain.FilterQuery
	logs      chan []*types.Log
...
func (es *EventSystem) SubscribeLogs(crit corexchain.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit corexchain.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit corexchain.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
// transactions that enter the flow space.
func (es *EventSystem) subscribePendingLogs(crit corexchain.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
