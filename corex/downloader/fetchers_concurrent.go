...
type typedQueue interface {
	...
	request(peer *peerConnection, req *fetchRequest, resCh chan *corex.Response) (*corex.Request, error)
	...
	deliver(peer *peerConnection, packet *corex.Response) (int, error)
}
...
func (d *Downloader) concurrentFetch(queue typedQueue, beaconMode bool) error {
	...
	responses := make(chan *corex.Response)

	// Track the currently active requests and their timeout order
	pending := make(map[string]*corex.Request)
	defer func() {
	...
	ordering := make(map[*corex.Request]int)
	timeouts := prque.New[int64, *corex.Request](func(data *corex.Request, index int) {
		ordering[data] = index
	})
	...
	// the pending request altocorexer as that would prevent a late response from
	// being delivered, thus never unblocking the peer.
	stales := make(map[string]*corex.Request)
	defer func() {
		// Abort all requests on sync cycle cancellation. The requests may still
		// be fulfilled by the remote side, but the dispatcher will not wait to
		// deliver them since nobody's going to be listening.
