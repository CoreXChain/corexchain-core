...
var (
	...
	maxQueuedHeaders            = 32 * 1024                         // [corex/62] Maximum number of headers to queue for import (DOS protection)
...
var (
	...
	ErrMergeTransition         = errors.New("classic sync reached the merge")
)
...
type Downloader struct {
	...
	stateDB corexdb.Database // Database to state sync into (and deduplicate via)
	...
	syncStartBlock uint64    // Head snap block when CoreX was started
...
func New(stateDb corexdb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn, success func()) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
// these are zero.
func (d *Downloader) Progress() corexchain.SyncProgress {
	...
	return corexchain.SyncProgress{
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td, ttd *big.Int, beaconMode bool) (err error) {
	...
	if !beaconMode {
		log.Debug("Synchronising with the network", "peer", p.id, "corex", p.version, "head", hash, "td", td, "mode", mode)
	} else {
	...
	for {
		...
		case task := <-d.headerProcCh:
			...
			if task == nil || len(task.headers) == 0 {
				...
				// If we're in classic sync mode, we need to check total difficulty
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	if index, err := d.blockchain.InsertChain(blocks); err != nil {
		if index < len(results) {
			...
			// The importer will put tocorexer a new list of blocks to import, which is a superset
