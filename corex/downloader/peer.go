...
type peerConnection struct {
	...
	version uint       // CoreX protocol version number to switch strategies
...
type Peer interface {
	...
	RequestHeadersByHash(common.Hash, int, int, bool, chan *corex.Response) (*corex.Request, error)
	RequestHeadersByNumber(uint64, int, int, bool, chan *corex.Response) (*corex.Request, error)

	RequestBodies([]common.Hash, chan *corex.Response) (*corex.Request, error)
	RequestReceipts([]common.Hash, chan *corex.Response) (*corex.Request, error)
}
...
func (p *peerConnection) UpdateHeaderRate(delivered int, elapsed time.Duration) {
	p.rates.Update(corex.BlockHeadersMsg, elapsed, delivered)
}
...
func (p *peerConnection) UpdateBodyRate(delivered int, elapsed time.Duration) {
	p.rates.Update(corex.BlockBodiesMsg, elapsed, delivered)
}
...
func (p *peerConnection) UpdateReceiptRate(delivered int, elapsed time.Duration) {
	p.rates.Update(corex.ReceiptsMsg, elapsed, delivered)
}

// HeaderCapacity retrieves the peer's header download allowance based on its
// previously discovered throughput.
func (p *peerConnection) HeaderCapacity(targetRTT time.Duration) int {
	cap := p.rates.Capacity(corex.BlockHeadersMsg, targetRTT)
...
func (p *peerConnection) BodyCapacity(targetRTT time.Duration) int {
	cap := p.rates.Capacity(corex.BlockBodiesMsg, targetRTT)
...
func (p *peerConnection) ReceiptCapacity(targetRTT time.Duration) int {
	cap := p.rates.Capacity(corex.ReceiptsMsg, targetRTT)
...
func newPeerSet() *peerSet {
	return &peerSet{
		...
		rates: msgrate.NewTrackers(log.New("proto", "corex")),
	}
}
