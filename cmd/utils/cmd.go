...
func StartNode(ctx *cli.Context, stack *node.Node, isConsole bool) {
	...
	go func() {
		...
		minFreeDiskSpace := 2 * corexconfig.Defaults.TrieDirtyCache // Default 2 * 256Mb
...
func monitorFreeDiskSpace(sigc chan os.Signal, path string, freeDiskSpaceCritical uint64) {
	...
	for {
		...
		if freeSpace < freeDiskSpaceCritical {
			log.Error("Low disk space. Gracefully shutting down CoreX to prevent database corruption.", "available", common.StorageSize(freeSpace), "path", path)
			sigc <- syscall.SIGTERM
			break
		} else if freeSpace < 2*freeDiskSpaceCritical {
			log.Warn("Disk space is running low. CoreX will shutdown if disk space runs below critical level.", "available", common.StorageSize(freeSpace), "critical_level", common.StorageSize(freeDiskSpaceCritical), "path", path)
		}
...
func ImportHistory(chain *core.BlockChain, db corexdb.Database, dir string, network string) error {
...
func ImportPreimages(db corexdb.Database, fn string) error {
...
// It's a part of the deprecated functionality, should be removed in the future.
func ExportPreimages(db corexdb.Database, fn string) error {
...
func ExportSnapshotPreimages(chaindb corexdb.Database, snaptree *snapshot.Tree, fn string, root common.Hash) error {
	log.Info("Exporting preimages", "file", fn)
...
type exportHeader struct {
	Magic    string // Always set to 'gcorexdbdump' for disambiguation
	Version  uint64
	Kind     string
	UnixTime uint64
}

const exportMagic = "gcorexdbdump"
const (
	OpBatchAdd = 0
	OpBatchDel = 1
)
...
func ImportLDBData(db corexdb.Database, f string, startIndex int64, interrupt chan struct{}) error {
