...
It's deprecated, please use "corex db import" instead.
`,
	}
...
func dumpGenesis(ctx *cli.Context) error {
	encodeGenesis := func(genesis *core.Genesis) error {
		enc := json.NewEncoder(os.Stdout)
		enc.SetIndent("", "  ")
		return enc.Encode(core.EnsureGenesisMetadata(genesis))
	}
	...
	if genesis != nil {
		if err := encodeGenesis(genesis); err != nil {
...
func importHistory(ctx *cli.Context) error {
	...
	if utils.IsNetworkPreset(ctx) {
		switch {
		case ctx.Bool(utils.CoreNetFlag.Name):
			network = "corenet"
		case ctx.Bool(utils.TestNetFlag.Name):
			network = "testnet"
		case ctx.Bool(utils.LabNetFlag.Name):
			network = "labnet"
		}
	} else {
...
func parseDumpConfig(ctx *cli.Context, stack *node.Node) (*state.DumpConfig, corexdb.Database, common.Hash, error) {
