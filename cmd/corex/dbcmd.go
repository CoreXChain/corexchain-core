...
func removeDB(ctx *cli.Context) error {
	...
	var (
		...
		ancientDir = config.CoreX.DatabaseFreezer
	)
	switch {
	case ancientDir == "":
...
func confirmAndRemoveDB(paths []string, kind string, ctx *cli.Context, removeFlagName string) {
	...
	if ctx.IsSet(removeFlagName) {
		...
		confirm, err = promptConfirm(fmt.Sprintf("Remove '%s'?", kind))
	}
	switch {
	case err != nil:
		utils.Fatalf("%v", err)
...
func showLeveldbStats(db corexdb.KeyValueStater) {
...
type snapshotIterator struct {
	init    bool
	account corexdb.Iterator
	storage corexdb.Iterator
}
...
var chainExporters = map[string]func(db corexdb.Database) utils.ChainDataIterator{
	"preimage": func(db corexdb.Database) utils.ChainDataIterator {
		iter := db.NewIterator(rawdb.PreimagePrefix, nil)
		return &preimageIterator{iter: iter}
	},
	"snapshot": func(db corexdb.Database) utils.ChainDataIterator {
