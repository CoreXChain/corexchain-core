...
import (
	...
	"com.corexkey/accounts/usbwallet/trezor"
	"com.corexkey/common"
...
// or whether the CoreXChain app was not started on it.
...
// CoreXChain address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.CoreXChainAddress)
	if _, err := w.trezorExchange(&trezor.CoreXChainGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.CoreXChainSignTx{
	...
	response := new(trezor.CoreXChainTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.CoreXChainTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
