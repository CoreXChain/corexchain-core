...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the CoreXChain
...
type wallet struct {
	...
	deriveChain     corexchain.ChainStateReader // Blockchain state reader to discover used account with
	...
	for errc == nil && err == nil {
		...
		for i := 0; i < len(nextAddrs); i++ {
			for empty := false; !empty; {
				// Retrieve the next derived CoreXChain account
				...
				if balance.Sign() == 0 && nonce == 0 {
					...
					// of historical-ledger, the first account on the historical path will
					// be shown to the user, even if we don't actively track it
					if i < len(nextAddrs)-1 {
						w.log.Info("Skipping tracking first account on historical path, use account.deriveAccount(<url>,<path>, false) to track",
...
// chain state reader.
func (w *wallet) SelfDerive(bases []accounts.DerivationPath, chain corexchain.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
