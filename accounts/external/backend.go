...
import (
	...
	"com.corexkey/log"
	"com.corexkey/rpc"
	"com.corexkey/signer/core/apitypes"
)
...
func (api *ExternalSigner) SelfDerive(bases []accounts.DerivationPath, chain corexchain.ChainStateReader) {
...
func (api *ExternalSigner) SignText(account accounts.Account, text []byte) ([]byte, error) {
	...
	if signature[64] == 27 || signature[64] == 28 {
		...
		// the signature to corexchain-type signature.
		signature[64] -= 27 // Transform V from CoreXChain-legacy to 0/1
	}
...
func (api *ExternalSigner) SignTx(account accounts.Account, tx *types.Transaction, chainID *big.Int) (*types.Transaction, error) {
	...
	switch tx.Type() {
	case types.DirectTxType, types.AccessProfileTxType:
