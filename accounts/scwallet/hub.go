...
//    You can pair with an unpaired wallet with `account.openWallet(URI, pairing password)`.
...
//    initialize a wallet with `account.initializeWallet(URI)`.
// 3. Connect to the wallet using the pairing information established in step 1.
//    You can connect to a paired wallet with `account.openWallet(URI, PIN)`.
