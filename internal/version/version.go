...
const ourPath = "com.corexkey" // Path to our module
...
// conventions in the CoreXChain p2p network.
...
//
// If the package that is currently executing is a prefixed by our corex-chain
...
func versionInfo(info *debug.BuildInfo) string {
	// If the main package is from our repo, prefix version with "corex".
	if strings.HasPrefix(info.Path, ourPath) {
		return fmt.Sprintf("corex %s", info.Main.Version)
	}
	...
	if mod == nil {
		...
		// version.
		return version + fmt.Sprintf("corex %s", params.VersionWithMeta)
	}
