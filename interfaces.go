...
// Package corexchain defines interfaces for interacting with CoreXChain.
package corexchain
...
// the CoreXChain network.
type SyncProgress struct {
	StartingBlock uint64 // Block number where sync began
	CurrentBlock  uint64 // Current block number where sync is at
	HighestBlock  uint64 // Highest alleged block number in the chain

	// "fast sync" fields. These used to be sent by corex, but are no longer used
	// since version v1.10.
...
type CallMsg struct {
	From         common.Address  // the sender of the call
	To           *common.Address // the destination contract (nil for contract creation)
	Gas          uint64          // if 0, the call executes with near-infinite gas
	ValueFlowFee *big.Int        // nano <-> execution resource exchange ratio
	GasFeeCap    *big.Int        // adaptive-fee ceiling per gas.
	GasTipCap    *big.Int        // adaptive-fee priority component per gas.
	Value        *big.Int        // amount of nano sent along with the call
	Data         []byte          // input data, usually an ABI-encoded contract method invocation

	AccessProfile types.AccessProfile // access profile for execution hints.
...
type FilterQuery struct {
	BlockHash *common.Hash     // used by corex_getLogs, return logs only from block with this hash
...
// signed transaction into the pending flow space for execution. If the transaction
...
// GasPricer wraps the value-flow fee oracle, which monitors the blockchain to determine the
// optimal value-flow fee given current fee market conditions.
type GasPricer interface {
	SuggestGasPrice(ctx context.Context) (*big.Int, error)
}

// ValueFlowPricerAdaptive provides access to the adaptive-fee value-flow oracle.
type ValueFlowPricerAdaptive interface {
