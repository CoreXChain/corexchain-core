...
var (
	...
	DefaultAuthModules = []string{"corex", "engine"}
)
...
var DefaultConfig = Config{
	...
	HTTPModules:          []string{"net", "corex", "communication", "pov", "trust", "asset", "valueflow", "account", "control", "identity"},
	HTTPVirtualHosts:     []string{"localhost"},
	HTTPTimeouts:         rpc.DefaultHTTPTimeouts,
	WSPort:               DefaultWSPort,
	WSModules:            []string{"net", "corex", "communication", "pov", "trust", "asset", "valueflow", "account", "control", "identity"},
...
func DefaultDataDir() string {
	...
	if home != "" {
		switch runtime.GOOS {
		case "darwin":
			return filepath.Join(home, "Library", "CoreXChain")
		case "windows":
			// We used to put everything in %HOME%\AppData\Roaming, but this caused
			// problems with non-typical setups. If this fallback location exists and
			// is non-empty, use it, otherwise DTRT and check %LOCALAPPDATA%.
			fallback := filepath.Join(home, "AppData", "Roaming", "CoreXChain")
			appdata := windowsAppData()
			if appdata == "" || isNonEmptyDir(fallback) {
				return fallback
			}
			return filepath.Join(appdata, "CoreXChain")
		default:
			return filepath.Join(home, ".corexchain")
		}
	}
