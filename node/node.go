...
func New(conf *Config) (*Node, error) {
	...
	// Creates an empty AccountManager with no backends. Callers (e.g. cmd/corex)
...
func (n *Node) startRPC() error {
	// Filter out any deprecated account bridge namespace unless it is explicitly enabled.
	var apis []rpc.API
	for _, api := range n.rpcAPIs {
		if api.Namespace == "accountbridge" {
			if n.config.EnableLegacyAccount {
				log.Warn("Deprecated account bridge namespace activated")
			} else {
				continue
			}
		}
...
func (n *Node) OpenDatabase(name string, cache, handles int, namespace string, readonly bool) (corexdb.Database, error) {
	n.lock.Lock()
	defer n.lock.Unlock()
	if n.state == closedState {
		return nil, ErrNodeStopped
...
func (n *Node) OpenDatabaseWithFreezer(name string, cache, handles int, ancient string, namespace string, readonly bool) (corexdb.Database, error) {
	n.lock.Lock()
	defer n.lock.Unlock()
	if n.state == closedState {
		return nil, ErrNodeStopped
...
func (n *Node) wrapDatabase(db corexdb.Database) corexdb.Database {
