...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of corex is "corex". If no
	...
	oldCoreXResourceWarning bool

	// AllowUnprotectedTxs allows non chain-protected protected transactions to be send over RPC.
	...
	// EnableLegacyAccount enables the deprecated account bridge.
	EnableLegacyAccount bool `toml:"-"`
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "CoreX", keep that.
	if name == "corex" || name == "corex-testnet" {
		name = "CoreX"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "corex" instances.
var isOldCoreXResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by corex 1.4 are used if they exist.
	if warn, isOld := isOldCoreXResource[path]; isOld {
		oldpath := ""
		if c.name() == "corex" {
			oldpath = filepath.Join(c.DataDir, path)
		}
		if oldpath != "" && common.FileExist(oldpath) {
			if warn && !c.oldCoreXResourceWarning {
				c.oldCoreXResourceWarning = true
				log.Warn("Using deprecated resource file, please move this file to the 'corex' subdirectory of datadir.", "file", oldpath)
			}
			return oldpath
		}
	}
...
func (c *Config) GetKeyStoreDir() (string, bool, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = os.MkdirTemp("", "corex-chain-keystore")
		isEphemeral = true
	}
