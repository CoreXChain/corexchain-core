...
type Sync struct {
	...
	database corexdb.KeyValueReader         // Persistent database to check for existing entries
...
func NewSync(root common.Hash, database corexdb.KeyValueReader, callback LeafCallback, scheme string) *Sync {
	ts := &Sync{
		scheme:   scheme,
		database: database,
...
func (s *Sync) AddSubTrie(root common.Hash, path []byte, parent common.Hash, parentPath []byte, callback LeafCallback) {
	...
	// If this sub-trie has a designated parent, link them tocorexer
...
// both corex/6x style fast sync and snap/1x style state sync, the paths of trie
...
func (s *Sync) Commit(dbw corexdb.Batch) error {
