...
// Package remotedb implements the key-value database layer based on a remote corex
// node. Under the hood, it utilises the `debug_dbGet` method to implement a
// read-only database.
// There really are no guarantees in this database, since the local corex does not
...
func (db *Database) ReadAncients(fn func(op corexdb.AncientReaderOp) error) (err error) {
	return fn(db)
}
...
func (db *Database) ModifyAncients(f func(corexdb.AncientWriteOp) error) (int64, error) {
	panic("not supported")
}
...
func (db *Database) NewBatch() corexdb.Batch {
	panic("not supported")
}

func (db *Database) NewBatchWithSize(size int) corexdb.Batch {
	panic("not supported")
}

func (db *Database) NewIterator(prefix []byte, start []byte) corexdb.Iterator {
	panic("not supported")
}
...
func (db *Database) NewSnapshot() (corexdb.Snapshot, error) {
	panic("not supported")
}
...
func New(client *rpc.Client) corexdb.Database {
	return &Database{
		remote: client,
	}
}
