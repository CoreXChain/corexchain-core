...
// Package p2p implements the CoreXChain p2p network protocols.
package p2p
...
func (srv *Server) setupUDPListening() (*net.UDPConn, error) {
	...
	if !laddr.IP.IsLoopback() && !laddr.IP.IsPrivate() {
		srv.portMappingRegister <- &portMapping{
			protocol: "UDP",
			name:     "corexchain peer discovery",
			port:     laddr.Port,
		}
	}
...
type NodeInfo struct {
	...
	ENR   string `json:"enr"`   // CoreXChain Node Record
