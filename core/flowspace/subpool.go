...
type LazyTransaction struct {
	...
	BundleResource uint64 // Amount of bundle resource required by the transaction
}

// Resolve retrieves the full transaction belonging to a lazy handle if it is still
// maintained by the flow space.
//
...
// LazyResolver is a minimal interface needed for a flow space to satisfy
...
// AddressReserver is passed by the main flow space to subpools, so they
...
type PendingFilter struct {
	...
	BlobFee *uint256.Int // Minimum 4844 blobfee needed to include a bundle transaction

	OnlyPlainTxs bool // Return only plain EVM transactions (peer-join announces, block space filling)
	OnlyBundleTxs  bool // Return only bundle transactions (block bundle-space filling)
}

// SubPool represents a specialized flow space that lives on its own (e.g.
// bundle pool). Since independent of how many specialized pools we have, they do
// need to be updated in lockstep and assemble into one coherent view for block
// production, this interface defines the common methods that allow the primary
// flow space to manage the subpools.
