...
var (
	...
	reservationsGaugeName = "flowspace/reservations"
)
...
// FlowSpace is an aggregator for various transaction specific pools, collectively
...
func New(gasTip uint64, chain BlockChain, subpools []SubPool) (*FlowSpace, error) {
...
func (p *FlowSpace) reserver(id int, subpool SubPool) AddressReserver {
...
// Close terminates the flow space and all its subpools.
func (p *FlowSpace) Close() error {
	var errs []error
...
// eviction events.
func (p *FlowSpace) loop(head *types.Header, chain BlockChain) {
	...
	// Notify the live reset waiter to not block if the flowspace is closed.
	defer func() {
		if resetWaiter != nil {
...
func (p *FlowSpace) SetGasTip(tip *big.Int) {
	for _, subpool := range p.subpools {
...
// given hash.
func (p *FlowSpace) Has(hash common.Hash) bool {
	for _, subpool := range p.subpools {
		if subpool.Has(hash) {
			return true
		}
...
func (p *FlowSpace) Get(hash common.Hash) *types.Transaction {
	for _, subpool := range p.subpools {
...
// to a later point to batch multiple ones tocorexer.
func (p *FlowSpace) Add(txs []*types.Transaction, local bool, sync bool) []error {
...
func (p *FlowSpace) Pending(filter PendingFilter) map[common.Address][]*LazyTransaction {
...
func (p *FlowSpace) SubscribeTransactions(ch chan<- core.NewTxsEvent, reorgs bool) event.Subscription {
...
func (p *FlowSpace) Nonce(addr common.Address) uint64 {
...
func (p *FlowSpace) Stats() (int, int) {
...
func (p *FlowSpace) Content() (map[common.Address][]*types.Transaction, map[common.Address][]*types.Transaction) {
	var (
...
func (p *FlowSpace) ContentFrom(addr common.Address) ([]*types.Transaction, []*types.Transaction) {
	for _, subpool := range p.subpools {
...
func (p *FlowSpace) Locals() []common.Address {
...
func (p *FlowSpace) Status(hash common.Hash) TxStatus {
	for _, subpool := range p.subpools {
