...
type limboBlob struct {
	...
	Block  uint64      // Block in which the bundle transaction was included
...
// newLimbo opens and indexes a set of limboed bundle transactions.
...
// push stores a new bundle transaction into the limbo, waiting until finality for
...
func (l *limbo) push(tx *types.Transaction, block uint64) error {
	...
	if _, ok := l.index[tx.Hash()]; ok {
		...
		return errors.New("already tracked bundle transaction")
	}
...
func (l *limbo) pull(tx common.Hash) (*types.Transaction, error) {
	...
	// can happen for example if a bundle transaction is mined without pushing it
	// into the network first.
	id, ok := l.index[tx]
	if !ok {
		log.Trace("Limbo cannot pull non-tracked blobs", "tx", tx)
		return nil, errors.New("unseen bundle transaction")
	}
...
// update changes the block number under which a bundle transaction is tracked. This
