...
// Package bundlepool implements the data-bundle blob flow space.
package bundlepool
...
const (
	...
	blobSize = params.BundleTxFieldElementsPerBlob * params.BundleTxBytesPerFieldElement

	// maxBundlesPerTransaction is the maximum number of blobs a single transaction
	...
	// the included blobs. Since bundle transactions are pulled instead of pushed,
	...
	// maxTxsPerAccount is the maximum number of bundle transactions admitted from
...
// bundleTxMeta is the minimal subset of types.BundleTx necessary to validate and
// schedule the bundle transactions into the following blocks. Only ever add the
// bare minimum needed fields to keep the size down (and thus number of entries
// larger with the same memory consumption).
type bundleTxMeta struct {
	...
	blobfeeJumps float64 // Absolute number of 4844 fee adjustments needed to reach the tx's bundle fee cap

	evictionExecTip      *uint256.Int // Worst priority fee across all previous nonces
	evictionExecFeeJumps float64      // Worst base fee (converted to fee jumps) across all previous nonces
	evictionBlobFeeJumps float64      // Worse bundle fee (converted to fee jumps) across all previous nonces
}

// newBundleTxMeta retrieves the indexed metadata fields from a bundle transaction
// and assembles a helper struct to track in memory.
func newBundleTxMeta(id uint64, size uint32, tx *types.Transaction) *bundleTxMeta {
	meta := &bundleTxMeta{
		hash:       tx.Hash(),
		id:         id,
		size:       size,
		...
		blobFeeCap: uint256.MustFromBig(tx.BundleValueFeeCap()),
		execGas:    tx.Gas(),
		blobGas:    tx.BundleResource(),
	}
...
// BundlePool is the flow space dedicated to data-bundle traffic.
//
...
// behavioural expectations allow us to design a flow space that is more robust
...
//     it wants to.
//
//   - Block bundle-space is limited. With blocks being capped to a few bundle txs, we
...
//
//   - Purpose of blobs are layer-2s. Layer-2s are meant to use bundle transactions to
//     commit to their own current state, which is independent of CoreXChain corenet
//     (state, txs). This means that there's no reason for bundle tx cancellation or
...
//     bundle transaction to an existing one should be aggressively discouraged.
//     Whilst generic transactions can start at 1 Nano gas cost and require a 10%
//     fee bump to replace, we suggest requiring a higher min cost (e.g. 1 gcorex)
//     and a more aggressive bump (100%).
//
//   - Cancellation is prohibitive. Evicting an already propagated bundle tx is a huge
//     DoS vector. As such, a) replacement (higher-fee) bundle txs mustn't invalidate
//     already propagated (future) bundle txs (cumulative fee); b) nonce-gapped blob
//     txs are disallowed; c) the presence of bundle transactions exclude non-bundle
//     transactions.
//
...
//     or flashbotter). The pool should cap the total number of bundle transactions
...
//     retain the 1559 2D gas pricing (and introduce on top a dynamic bundle resource fee),
//     so locality is moot. With a disk backed bundle pool avoiding the resend issue,
...
//     bundle txs containing 0 blobs. In practice, admitting such txs into the pool
...
//     them back into the bundle pool, which can break invariants.
//
...
//     so a mini reorg cannot re-pool "lost" bundle transactions. To support reorgs,
//     blobs are retained on disk until they are finalised.
//
//   - Blobs can arrive via flashbots. Blocks might contain bundle transactions we
...
// that the bundle pool should work robustly for its intended use case and whilst
// anyone is free to use bundle transactions for arbitrary non-rollup use cases,
...
//     paying bundle transaction anywhere in the pooled nonce-sequence, as that tx
...
//
//   - Since bundle transactions have 3 price parameters: execution tip, execution
//     fee cap and data fee cap, there's no singular parameter to create a total
//     price ordering on. What's more, since the base fee and bundle fee can move
...
//
//   - The first observation is that comparing 1559 base fees or 4844 bundle fees
...
//     B has higher bundle fee). Rather than look at the absolute fees, the useful
...
//
//   - For the scenario where the pool does not contain non-executable bundle txs
...
//     priority will be grouped tocorexer.
//
...
type BundlePool struct {
	...
	reserve flowspace.AddressReserver // Address reserver to ensure exclusivity across subpools
	...
	gasTip *uint256.Int   // Currently accepted minimum priority fee

	lookup map[common.Hash]uint64             // Lookup table mapping hashes to tx billy entries
	index  map[common.Address][]*bundleTxMeta // Bundle transactions grouped by accounts, sorted by nonce
...
// New creates a new blob flow space to gather, sort and filter inbound
// bundle transactions from the network.
func New(config Config, chain BlockChain) *BundlePool {
	// Sanitize the input to ensure no vulnerable value-flow fees are set
	config = (&config).sanitize()

	// Create the flow space with its initial settings
	return &BundlePool{
		config: config,
		signer: types.LatestSigner(chain.Config()),
		chain:  chain,
		lookup: make(map[common.Hash]uint64),
		index:  make(map[common.Address][]*bundleTxMeta),
		spent:  make(map[common.Address]*uint256.Int),
	}
}

// Filter returns whether the given transaction can be consumed by the bundle pool.
func (p *BundlePool) Filter(tx *types.Transaction) bool {
	return tx.Type() == types.BundleTxType
}

// Init sets the value-flow fee needed to keep a transaction in the pool and the chain
// head to allow balance / nonce checks. The transaction journal will be loaded
// from disk and filtered based on the provided starting settings.
func (p *BundlePool) Init(gasTip uint64, head *types.Header, reserve flowspace.AddressReserver) error {
	p.reserve = reserve
	...
	if len(fails) > 0 {
		log.Warn("Dropping invalidated bundle transactions", "ids", fails)
	...
	var (
		...
		blobfee = uint256.NewInt(params.BundleTxMinBundleResourceprice)
	)
	if p.head.ExcessBundleResource != nil {
		blobfee = uint256.MustFromBig(eip4844.CalcBlobFee(*p.head.ExcessBundleResource))
	}
	...
	// Set the configured priority fee, triggering a filtering of anything just loaded
...
func (p *BundlePool) parseTransaction(id uint64, size uint32, blob []byte) error {
	tx := new(types.Transaction)
	if err := rlp.DecodeBytes(blob, tx); err != nil {
		// This path is impossible unless the disk data representation changes
		// across restarts. For that ever improbable case, recover gracefully
		// by ignoring this data entry.
		log.Error("Failed to decode bundle pool entry", "id", id, "err", err)
		return err
	}
	if tx.BundleAttachment() == nil {
		log.Error("Missing attachment in bundle pool entry", "id", id, "hash", tx.Hash())
		return errors.New("missing blob attachment")
	}

	meta := newBundleTxMeta(id, size, tx)
	if _, exists := p.lookup[meta.hash]; exists {
		// This path is only possible after a crash, where deleted items are not
		// removed via the normal shutdown-startup procedure and thus may get
		// partially resurrected.
		log.Error("Rejecting duplicate bundle pool entry", "id", id, "hash", tx.Hash())
	...
	if err != nil {
		...
		// this data entry.
		log.Error("Failed to recover bundle tx sender", "id", id, "hash", tx.Hash(), "err", err)
		return err
	}
...
func (p *BundlePool) recheck(addr common.Address, inclusions map[common.Hash]uint64) {
	...
	// If there is a gap between the chain state and the bundle pool, drop
	...
	if gapped || filled {
		...
		if gapped {
			log.Warn("Dropping dangling bundle transactions", "from", addr, "missing", next, "drop", nonces, "ids", ids)
			dropDanglingMeter.Mark(int64(len(ids)))
		} else {
			log.Trace("Dropping filled bundle transactions", "from", addr, "filled", nonces, "ids", ids)
			dropFilledMeter.Mark(int64(len(ids)))
		}
		for _, id := range ids {
			if err := p.store.Delete(id); err != nil {
				log.Error("Failed to delete bundle transaction", "from", addr, "id", id, "err", err)
			}
		}
		return
	}
	// If there is overlap between the chain state and the bundle pool, drop
	...
	if txs[0].nonce < next {
		...
		log.Trace("Dropping overlapped bundle transactions", "from", addr, "overlapped", nonces, "ids", ids, "left", len(txs))
	...
	for i := 1; i < len(txs); i++ {
		...
		//
		// Also, Billy behind the bundlepool does not journal deletes. A process
		...
		if txs[i].nonce == txs[i-1].nonce {
			...
			log.Error("Dropping repeat nonce bundle transaction", "from", addr, "nonce", txs[i].nonce, "id", id)
		...
		log.Error("Dropping gapped bundle transactions", "from", addr, "missing", txs[i-1].nonce+1, "drop", nonces, "ids", ids)
	...
	if spent.Cmp(balance) > 0 {
		...
		log.Warn("Dropping overdrafted bundle transactions", "from", addr, "balance", balance, "spent", spent, "drop", nonces, "ids", ids)
	...
	if len(txs) > maxTxsPerAccount {
		...
		log.Warn("Dropping overcapped bundle transactions", "from", addr, "kept", len(txs), "drop", nonces, "ids", ids)
...
// offload removes a tracked bundle transaction from the pool and moves it into the
...
func (p *BundlePool) offload(addr common.Address, nonce uint64, id uint64, inclusions map[common.Hash]uint64) {
	...
	if err := p.limbo.push(&tx, block); err != nil {
		log.Warn("Failed to offload bundle tx into limbo", "err", err)
		return
	}
}

// Reset implements flowspace.SubPool, allowing the bundle pool's internal state to be
// kept in sync with the main flow space's internal state.
func (p *BundlePool) Reset(oldHead, newHead *types.Header) {
	...
	if err != nil {
		log.Error("Failed to reset bundlepool state", "err", err)
		return
	}
	p.head = newHead
	p.state = statedb
		...
		for addr, txs := range reinject {
			...
			for _, tx := range txs {
				if err := p.reinject(addr, tx.Hash()); err == nil {
					adds = append(adds, tx.WithoutBundleAttachment())
				}
			}
	...
	var (
		...
		blobfee = uint256.MustFromBig(big.NewInt(params.BundleTxMinBundleResourceprice))
	)
	if newHead.ExcessBundleResource != nil {
		blobfee = uint256.MustFromBig(eip4844.CalcBlobFee(*newHead.ExcessBundleResource))
	}
...
func (p *BundlePool) reorg(oldHead, newHead *types.Header) (map[common.Address][]*types.Transaction, map[common.Hash]uint64) {
	...
	if rem == nil {
		...
		// being the cause
		log.Debug("Skipping bundlepool reset caused by setHead",
			"old", oldHead.Hash(), "oldnum", oldNum, "new", newHead.Hash(), "newnum", newNum)
		return nil, nil
	}
	...
	for rem.NumberU64() > add.NumberU64() {
		...
		if rem = p.chain.GetBlock(rem.ParentHash(), rem.NumberU64()-1); rem == nil {
			log.Error("Unrooted old chain seen by bundlepool", "block", oldHead.Number, "hash", oldHead.Hash())
			return nil, nil
		}
	}
	for add.NumberU64() > rem.NumberU64() {
		...
		if add = p.chain.GetBlock(add.ParentHash(), add.NumberU64()-1); add == nil {
			log.Error("Unrooted new chain seen by bundlepool", "block", newHead.Number, "hash", newHead.Hash())
			return nil, nil
		}
	}
...
func (p *BundlePool) reinject(addr common.Address, txhash common.Hash) error {
	...
	meta := newBundleTxMeta(id, p.store.Size(id), tx)
	if _, ok := p.index[addr]; !ok {
		if err := p.reserve(addr, true); err != nil {
			log.Warn("Failed to reserve account for bundle pool", "tx", tx.Hash(), "from", addr, "err", err)
			return err
		}
		p.index[addr] = []*bundleTxMeta{meta}
...
// SetGasTip implements flowspace.SubPool, allowing the bundle pool's gas requirements
// to be kept in sync with the main flow space's gas requirements.
func (p *BundlePool) SetGasTip(tip *big.Int) {
	p.lock.Lock()
	defer p.lock.Unlock()

	// Store the new minimum priority fee
	old := p.gasTip
		...
		for addr, txs := range p.index {
			for i, tx := range txs {
				if tx.execTipCap.Cmp(p.gasTip) < 0 {
					...
					log.Warn("Dropping underpriced bundle transaction", "from", addr, "rejected", tx.nonce, "tip", tx.execTipCap, "want", tip, "drop", nonces, "ids", ids)
...
func (p *BundlePool) validateTx(tx *types.Transaction) error {
	// Ensure the transaction adheres to basic pool filters (type, size, tip) and
	// consensus rules
	baseOpts := &flowspace.ValidationOptions{
		Config:  p.chain.Config(),
		Accept:  1 << types.BundleTxType,
		MaxSize: txMaxSize,
		MinTip:  p.gasTip.ToBig(),
	}
	if err := flowspace.ValidateTransaction(tx, p.head, p.signer, baseOpts); err != nil {
		return err
	}
	// Ensure the transaction adheres to the stateful pool filters (nonce, balance)
	stateOpts := &flowspace.ValidationOptionsWithState{
		State: p.state,

		FirstNonceGap: func(addr common.Address) uint64 {
			// Nonce gaps are not permitted in the bundle pool, the first gap will
	...
	if err := flowspace.ValidateTransactionWithState(tx, p.signer, stateOpts); err != nil {
		return err
	}
	...
	if uint64(len(p.index[from])) > tx.Nonce()-next {
		...
		case tx.GasFeeCapIntCmp(prev.execFeeCap.ToBig()) <= 0:
			return fmt.Errorf("%w: new tx gas fee cap %v <= %v queued", flowspace.ErrReplaceUnderpriced, tx.GasFeeCap(), prev.execFeeCap)
		case tx.GasTipCapIntCmp(prev.execTipCap.ToBig()) <= 0:
			return fmt.Errorf("%w: new tx priority fee cap %v <= %v queued", flowspace.ErrReplaceUnderpriced, tx.GasTipCap(), prev.execTipCap)
		case tx.BundleValueFeeCapIntCmp(prev.blobFeeCap.ToBig()) <= 0:
			return fmt.Errorf("%w: new tx bundle resource fee cap %v <= %v queued", flowspace.ErrReplaceUnderpriced, tx.BundleValueFeeCap(), prev.blobFeeCap)
		}
		var (
			...
			minBundleValueFeeCap = new(uint256.Int).Div(new(uint256.Int).Mul(multiplier, prev.blobFeeCap), onehundred)
		)
		switch {
		case tx.GasFeeCapIntCmp(minGasFeeCap.ToBig()) < 0:
			return fmt.Errorf("%w: new tx gas fee cap %v <= %v queued + %d%% replacement penalty", flowspace.ErrReplaceUnderpriced, tx.GasFeeCap(), prev.execFeeCap, p.config.PriceBump)
		case tx.GasTipCapIntCmp(minGasTipCap.ToBig()) < 0:
			return fmt.Errorf("%w: new tx priority fee cap %v <= %v queued + %d%% replacement penalty", flowspace.ErrReplaceUnderpriced, tx.GasTipCap(), prev.execTipCap, p.config.PriceBump)
		case tx.BundleValueFeeCapIntCmp(minBundleValueFeeCap.ToBig()) < 0:
			return fmt.Errorf("%w: new tx bundle resource fee cap %v <= %v queued + %d%% replacement penalty", flowspace.ErrReplaceUnderpriced, tx.BundleValueFeeCap(), prev.blobFeeCap, p.config.PriceBump)
		}
	}
	return nil
}
...
func (p *BundlePool) Get(hash common.Hash) *types.Transaction {
	// Track the amount of time waiting to retrieve a fully resolved bundle tx from
	...
	data, err := p.store.Get(id)
	if err != nil {
		log.Error("Tracked bundle transaction missing from store", "hash", hash, "id", id, "err", err)
		return nil
	}
...
// Add inserts a set of bundle transactions into the pool if they pass validation (both
...
// Add inserts a new bundle transaction into the pool if it passes validation (both
// consensus validity and pool restrictions).
func (p *BundlePool) add(tx *types.Transaction) (err error) {
	// The bundle pool blocks on adding a transaction. This is because bundle txs are
	...
	if err := p.validateTx(tx); err != nil {
		...
		switch {
		case errors.Is(err, flowspace.ErrUnderpriced):
		...
		case errors.Is(err, flowspace.ErrAccountLimitExceeded):
			addOvercappedMeter.Mark(1)
		case errors.Is(err, flowspace.ErrReplaceUnderpriced):
...
func (p *BundlePool) drop() {
	...
	log.Debug("Evicting overflown bundle transaction", "from", from, "evicted", drop.nonce, "id", drop.id)
...
//
// The transactions can also be pre-filtered by the adaptive fee components to
// reduce allocations and load on downstream subsystems.
func (p *BundlePool) Pending(filter flowspace.PendingFilter) map[common.Address][]*flowspace.LazyTransaction {
	// If only plain transactions are requested, this pool is unsuitable as it
	// contains none, don't even bother.
	if filter.OnlyPlainTxs {
		return nil
	}
	// Track the amount of time waiting to retrieve the list of pending bundle txs
	...
	pending := make(map[common.Address][]*flowspace.LazyTransaction, len(p.index))
	for addr, txs := range p.index {
		lazies := make([]*flowspace.LazyTransaction, 0, len(txs))
		for _, tx := range txs {
			...
			lazies = append(lazies, &flowspace.LazyTransaction{
				Pool:           p,
				Hash:           tx.hash,
...
// them out as metrics.
func (p *BundlePool) updateStorageMetrics() {
	stats := p.store.Infos()
	...
	for _, shelf := range stats.Shelves {
		...
		if shelf.SlotSize/blobSize > maxBundlesPerTransaction {
...
// them out as metrics.
func (p *BundlePool) updateLimboMetrics() {
...
func (p *BundlePool) Stats() (int, int) {
	...
	return pending, 0 // No non-executable txs in the bundle pool
}

// Content retrieves the data content of the flow space, returning all the
// pending as well as queued transactions, grouped by account and sorted by nonce.
//
// For the bundle pool, this method will return nothing for now.
...
// ContentFrom retrieves the data content of the flow space, returning the
...
//
// There is no notion of local accounts in the bundle pool.
...
func (p *BundlePool) Status(hash common.Hash) flowspace.TxStatus {
	if p.Has(hash) {
		return flowspace.TxStatusPending
	}
	return flowspace.TxStatusUnknown
}
