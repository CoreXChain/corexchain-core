...
type evictHeap struct {
	metas *map[common.Address][]*bundleTxMeta // Pointer to the bundle pool's index for price retrievals

	basefeeJumps float64 // Pre-calculated absolute adaptive fee jumps for the base fee
	blobfeeJumps float64 // Pre-calculated absolute adaptive fee jumps for the bundle fee
...
// newPriceHeap creates a new heap of cheapest accounts in the bundle pool to evict
// from in case of over saturation.
func newPriceHeap(basefee *uint256.Int, blobfee *uint256.Int, index *map[common.Address][]*bundleTxMeta) *evictHeap {
	heap := &evictHeap{
		metas: index,
...
// reinit updates the pre-calculated adaptive fee jumps in the price heap and runs
...
func (h *evictHeap) reinit(basefee *uint256.Int, blobfee *uint256.Int, force bool) {
	...
	// One or both of the adaptive fees jumped, resort the pool
