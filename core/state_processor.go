...
// Process processes the state changes according to the CoreXChain rules by running
...
func applyTransaction(msg *Message, config *params.ChainConfig, gp *GasPool, statedb *state.StateDB, blockNumber *big.Int, blockHash common.Hash, tx *types.Transaction, usedGas *uint64, evm *vm.EVM) (*types.Receipt, error) {
	...
	*usedGas += result.UsedGas
	...
	if tx.Type() == types.BundleTxType {
		receipt.BundleResourceUsed = uint64(len(tx.BundleHashes()) * params.BundleTxBundleResourcePerBlob)
		receipt.BundleValueFee = evm.Context.BundleBaseFee
	}
...
func ProcessBeaconBlockRoot(beaconRoot common.Hash, vmenv *vm.EVM, statedb *state.StateDB) {
	...
	statedb.AddAddressToAccessProfile(params.BeaconRootsStorageAddress)
