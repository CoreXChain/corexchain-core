...
// corexhash tests, please use OffsetTime, which implicitly recalculates the diff.
...
func (b *BlockGen) addTx(bc *BlockChain, vmConfig vm.Config, tx *types.Transaction) {
	...
	if b.header.BundleResourceUsed != nil {
		*b.header.BundleResourceUsed += receipt.BundleResourceUsed
	}
}
...
// BaseFee returns the adaptive fee base fee of the block being generated.
...
func GenerateChain(config *params.ChainConfig, parent *types.Block, engine consensus.Engine, db corexdb.Database, n int, gen func(int, *BlockGen)) ([]*types.Block, []types.Receipts) {
	if config == nil {
	...
	for i := 0; i < n; i++ {
		...
		if block.ExcessBundleResource() != nil {
			blobGasPrice = eip4844.CalcBlobFee(*block.ExcessBundleResource())
		}
...
func GenerateChainWithGenesis(genesis *Genesis, engine consensus.Engine, n int, gen func(int, *BlockGen)) (corexdb.Database, []*types.Block, []types.Receipts) {
...
func (cm *chainMaker) makeHeader(parent *types.Block, state *state.StateDB, engine consensus.Engine) *types.Header {
	...
	if cm.config.IsCancun(header.Number, header.Time) {
		var (
			parentExcessBundleResource uint64
			parentBundleResourceUsed   uint64
		)
		if parent.ExcessBundleResource() != nil {
			parentExcessBundleResource = *parent.ExcessBundleResource()
			parentBundleResourceUsed = *parent.BundleResourceUsed()
		}
		excessBundleResource := eip4844.CalcExcessBundleResource(parentExcessBundleResource, parentBundleResourceUsed)
		header.ExcessBundleResource = &excessBundleResource
		header.BundleResourceUsed = new(uint64)
...
func makeHeaderChain(chainConfig *params.ChainConfig, parent *types.Header, n int, engine consensus.Engine, db corexdb.Database, seed int) []*types.Header {
...
func makeHeaderChainWithGenesis(genesis *Genesis, n int, engine consensus.Engine, seed int) (corexdb.Database, []*types.Header) {
...
func makeBlockChain(chainConfig *params.ChainConfig, parent *types.Block, n int, engine consensus.Engine, db corexdb.Database, seed int) []*types.Block {
...
func makeBlockChainWithGenesis(genesis *Genesis, n int, engine consensus.Engine, seed int) (corexdb.Database, []*types.Block) {
