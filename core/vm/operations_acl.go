...
func makeGasSStoreFunc(clearingRefund uint64) gasFunc {
	return func(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
		...
		if addrPresent, slotPresent := evm.StateDB.SlotInAccessProfile(contract.Address(), slot); !slotPresent {
			cost = params.ColdSloadCostEIP2929
			// If the caller cannot afford the cost, this change will be rolled back
			evm.StateDB.AddSlotToAccessProfile(contract.Address(), slot)
			if !addrPresent {
				// Once we're done with YOLOv2 and schedule this for corenet, might
...
func gasSLoadEIP2929(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	// Check slot presence in the access list
	if _, slotPresent := evm.StateDB.SlotInAccessProfile(contract.Address(), slot); !slotPresent {
		// If the caller cannot afford the cost, this change will be rolled back
...
func gasExtCodeCopyEIP2929(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	// Check slot presence in the access list
	if !evm.StateDB.AddressInAccessProfile(addr) {
		evm.StateDB.AddAddressToAccessProfile(addr)
		var overflow bool
...
func makeCallVariantGasCallEIP2929(oldCalculator gasFunc) gasFunc {
	return func(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
		...
		// Check slot presence in the access list
		warmAccess := evm.StateDB.AddressInAccessProfile(addr)
...
func makeSelfdestructGasFn(refundsEnabled bool) gasFunc {
	gasFunc := func(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
		...
		if !evm.StateDB.AddressInAccessProfile(address) {
			// If the caller cannot afford the cost, this change will be rolled back
