...
//
// It can take several hours(around 2 hours for corenet) to finish
...
func NewPruner(db corexdb.Database, config Config) (*Pruner, error) {
...
func prune(snaptree *snapshot.Tree, root common.Hash, maindb corexdb.Database, stateBloom *stateBloom, bloomPath string, middleStateRoots map[common.Hash]struct{}, start time.Time) error {
...
func (p *Pruner) Prune(root common.Hash) error {
	...
	if !rawdb.HasLegacyTrieNode(p.db, root) {
		// The special case is for clique based networks(labnet
...
// in the disk.
func RecoverPruning(datadir string, db corexdb.Database) error {
...
func extractGenesis(db corexdb.Database, stateBloom *stateBloom) error {
