...
type Database interface {
	...
	DiskDB() corexdb.KeyValueStore

	// TrieDB returns the underlying trie database for managing trie nodes.
	TrieDB() *triedb.Database
}

// Trie is a CoreXChain Merkle Patricia trie.
type Trie interface {
	...
	Prove(key []byte, proofDb corexdb.KeyValueWriter) error
}

// NewDatabase creates a backing store for state. The returned database is safe for
// concurrent use, but does not retain any recent trie nodes in memory. To keep some
// historical state in memory, use the NewDatabaseWithConfig constructor.
func NewDatabase(db corexdb.Database) Database {
...
// large memory cache.
func NewDatabaseWithConfig(db corexdb.Database, config *triedb.Config) Database {
	return &cachingDB{
		disk:          db,
...
func NewDatabaseWithNodeDB(db corexdb.Database, triedb *triedb.Database) Database {
	return &cachingDB{
		disk:          db,
		codeSizeCache: lru.NewCache[common.Hash, int](codeSizeCacheSize),
		codeCache:     lru.NewSizeConstrainedCache[common.Hash, []byte](codeCacheSize),
...
// DiskDB returns the underlying key-value disk database.
func (db *cachingDB) DiskDB() corexdb.KeyValueStore {
	return db.disk
}
