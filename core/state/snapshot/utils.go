...
func CheckDanglingStorage(chaindb corexdb.KeyValueStore) error {
...
func checkDanglingDiskStorage(chaindb corexdb.KeyValueStore) error {
	var (
		lastReport = time.Now()
		start      = time.Now()
		lastKey    []byte
...
// snapshot difflayers.
func checkDanglingMemStorage(db corexdb.KeyValueStore) error {
	start := time.Now()
...
func CheckJournalAccount(db corexdb.KeyValueStore, hash common.Hash) error {
