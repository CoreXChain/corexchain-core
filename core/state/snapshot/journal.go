...
func loadAndParseJournal(db corexdb.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
	...
	// not existent, e.g. the disk layer is generating while that the CoreX
...
func loadSnapshot(diskdb corexdb.KeyValueStore, triedb *triedb.Database, root common.Hash, cache int, recovery bool, noBuild bool) (snapshot, bool, error) {
	...
	//
	// Possible scenario: CoreX was crashed without persisting journal and then
...
func iterateJournal(db corexdb.KeyValueReader, callback journalCallback) error {
