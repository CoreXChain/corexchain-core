...
func generateSnapshot(diskdb corexdb.KeyValueStore, triedb *triedb.Database, cache int, root common.Hash) *diskLayer {
...
func journalProgress(db corexdb.KeyValueWriter, marker []byte, stats *generatorStats) {
...
func (dl *diskLayer) checkAndFlush(ctx *generatorContext, current []byte) error {
	...
	if ctx.batch.ValueSize() > corexdb.IdealBatchSize || abort != nil {
