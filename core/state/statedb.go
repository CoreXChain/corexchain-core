...
// Package state provides a caching layer atop the CoreXChain state trie.
package state
...
import (
	...
	"com.corexkey/params"
	"com.corexkey/trie"
	"com.corexkey/trie/trienode"
	"com.corexkey/trie/triestate"
...
// StateDB structs within the corexchain protocol are used to store anything
...
//
// Carrying over the balance ensures that CoreX doesn't disappear.
...
func (s *StateDB) Copy() *StateDB {
	...
	for addr := range s.journal.dirties {
		// As documented [here](https://corex.internal/corex-chain-repo/pull/16485#issuecomment-380438527),
	...
	state.accessProfile = s.accessProfile.Copy()
...
func (s *StateDB) handleDestruction(nodes *trienode.MergedNodeSet) (map[common.Address]struct{}, error) {
	// Short circuit if corex is running with hash mode. This procedure can consume
...
func (s *StateDB) Prepare(rules params.Rules, sender, coinbase common.Address, dst *common.Address, precompiles []common.Address, list types.AccessProfile) {
	if rules.IsBerlin {
...
// AddAddressToAccessProfile adds the given address to the access list
func (s *StateDB) AddAddressToAccessProfile(addr common.Address) {
	if s.accessProfile.AddAddress(addr) {
		s.journal.append(accessProfileAddAccountChange{&addr})
	}
}

// AddSlotToAccessProfile adds the given (address, slot)-tuple to the access list
func (s *StateDB) AddSlotToAccessProfile(addr common.Address, slot common.Hash) {
	addrMod, slotMod := s.accessProfile.AddSlot(addr, slot)
	if addrMod {
	...
	if slotMod {
		s.journal.append(accessProfileAddSlotChange{
			address: &addr,
			slot:    &slot,
		})
	}
...
// AddressInAccessProfile returns true if the given address is in the access list.
func (s *StateDB) AddressInAccessProfile(addr common.Address) bool {
	return s.accessProfile.ContainsAddress(addr)
}

// SlotInAccessProfile returns true if the given (address, slot)-tuple is in the access list.
func (s *StateDB) SlotInAccessProfile(addr common.Address, slot common.Hash) (addressPresent bool, slotPresent bool) {
	return s.accessProfile.Contains(addr, slot)
}
