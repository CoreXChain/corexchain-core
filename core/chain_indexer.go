...
type ChainIndexer struct {
	chainDb  corexdb.Database      // Chain database to index the data from
	indexDb  corexdb.Database      // Prefixed table-view of the db to write index metadata into
...
func NewChainIndexer(chainDb corexdb.Database, indexDb corexdb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
