...
// Package types contains data types related to CoreXChain consensus.
package types
...
// Header represents a block header in the CoreXChain blockchain.
type Header struct {
	ParentHash  common.Hash    `json:"parentHash"       gencodec:"required"`
	UncleHash   common.Hash    `json:"sha3Uncles"       gencodec:"required"`
	Coinbase    common.Address `json:"producer"`
	...
	// BaseFee was added by adaptive fee and is ignored in legacy headers.
	BaseFee *big.Int `json:"baseFeePerGas" rlp:"optional"`

	// WithdrawalsHash was added by EIP-4895 and is ignored in legacy headers.
	WithdrawalsHash *common.Hash `json:"withdrawalsRoot" rlp:"optional"`

	// BundleResourceUsed was added by data-bundle and is ignored in legacy headers.
	BundleResourceUsed *uint64 `json:"bundleResourceUsed" rlp:"optional"`

	// ExcessBundleResource was added by data-bundle and is ignored in legacy headers.
	ExcessBundleResource *uint64 `json:"excessBundleResource" rlp:"optional"`
...
type headerMarshaling struct {
	...
	BundleResourceUsed   *hexutil.Uint64
	ExcessBundleResource *hexutil.Uint64
}
...
// a block's data contents (transactions and uncles) tocorexer.
type Body struct {
...
// Block represents a CoreXChain block.
//
...
type Block struct {
	...
	// These fields are used by package corex to track
	// inter-peer block relay.
	ReceivedAt   time.Time
	ReceivedFrom interface{}
}

// "external" block encoding. used for corex protocol, etc.
...
func CopyHeader(h *Header) *Header {
	...
	if h.ExcessBundleResource != nil {
		cpy.ExcessBundleResource = new(uint64)
		*cpy.ExcessBundleResource = *h.ExcessBundleResource
	}
	if h.BundleResourceUsed != nil {
		cpy.BundleResourceUsed = new(uint64)
		*cpy.BundleResourceUsed = *h.BundleResourceUsed
	}
...
func (b *Block) ExcessBundleResource() *uint64 {
	var excessBundleResource *uint64
	if b.header.ExcessBundleResource != nil {
		excessBundleResource = new(uint64)
		*excessBundleResource = *b.header.ExcessBundleResource
	}
	return excessBundleResource
}

func (b *Block) BundleResourceUsed() *uint64 {
	var bundleResourceUsed *uint64
	if b.header.BundleResourceUsed != nil {
		bundleResourceUsed = new(uint64)
		*bundleResourceUsed = *b.header.BundleResourceUsed
	}
	return bundleResourceUsed
}
