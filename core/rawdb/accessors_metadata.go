...
func ReadDatabaseVersion(db corexdb.KeyValueReader) *uint64 {
	var version uint64
...
func WriteDatabaseVersion(db corexdb.KeyValueWriter, version uint64) {
...
func ReadChainConfig(db corexdb.KeyValueReader, hash common.Hash) *params.ChainConfig {
...
func WriteChainConfig(db corexdb.KeyValueWriter, hash common.Hash, cfg *params.ChainConfig) {
	if cfg == nil {
		return
	}
...
func ReadGenesisStateSpec(db corexdb.KeyValueReader, blockhash common.Hash) []byte {
	data, _ := db.Get(genesisStateSpecKey(blockhash))
	return data
}

// WriteGenesisStateSpec writes the genesis state specification into the disk.
func WriteGenesisStateSpec(db corexdb.KeyValueWriter, blockhash common.Hash, data []byte) {
...
func PushUncleanShutdownMarker(db corexdb.KeyValueStore) ([]uint64, uint64, error) {
...
func PopUncleanShutdownMarker(db corexdb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
...
func UpdateUncleanShutdownMarker(db corexdb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		log.Warn("Error reading unclean shutdown markers", "error", err)
...
func ReadTransitionStatus(db corexdb.KeyValueReader) []byte {
	data, _ := db.Get(transitionStatusKey)
	return data
}

// WriteTransitionStatus stores the eth2 transition status to the database
func WriteTransitionStatus(db corexdb.KeyValueWriter, data []byte) {
