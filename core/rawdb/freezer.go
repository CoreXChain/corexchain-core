...
//     reserving it for corex-chain. This would also reduce the memory requirements
//     of CoreX, and thus also GC overhead.
type Freezer struct {
...
func (f *Freezer) ReadAncients(fn func(corexdb.AncientReaderOp) error) (err error) {
	f.writeLock.RLock()
	defer f.writeLock.RUnlock()
...
func (f *Freezer) ModifyAncients(fn func(corexdb.AncientWriteOp) error) (writeSize int64, err error) {
	if f.readonly {
