...
func ReadPreimage(db corexdb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(preimageKey(hash))
	return data
}

// WritePreimages writes the provided set of preimages to the database.
func WritePreimages(db corexdb.KeyValueWriter, preimages map[common.Hash][]byte) {
...
func ReadCode(db corexdb.KeyValueReader, hash common.Hash) []byte {
...
func ReadCodeWithPrefix(db corexdb.KeyValueReader, hash common.Hash) []byte {
...
func HasCode(db corexdb.KeyValueReader, hash common.Hash) bool {
	// Try with the prefixed code scheme first, if not then try with legacy
	// scheme.
...
func HasCodeWithPrefix(db corexdb.KeyValueReader, hash common.Hash) bool {
	ok, _ := db.Has(codeKey(hash))
	return ok
}

// WriteCode writes the provided contract code database.
func WriteCode(db corexdb.KeyValueWriter, hash common.Hash, code []byte) {
...
func DeleteCode(db corexdb.KeyValueWriter, hash common.Hash) {
...
func ReadStateID(db corexdb.KeyValueReader, root common.Hash) *uint64 {
...
func WriteStateID(db corexdb.KeyValueWriter, root common.Hash, id uint64) {
	var buff [8]byte
...
func DeleteStateID(db corexdb.KeyValueWriter, root common.Hash) {
...
func ReadPersistentStateID(db corexdb.KeyValueReader) uint64 {
...
func WritePersistentStateID(db corexdb.KeyValueWriter, number uint64) {
...
// the last shutdown.
func ReadTrieJournal(db corexdb.KeyValueReader) []byte {
...
// shutdown.
func WriteTrieJournal(db corexdb.KeyValueWriter, journal []byte) {
...
// the last shutdown.
func DeleteTrieJournal(db corexdb.KeyValueWriter) {
...
// state).
func ReadStateHistoryMeta(db corexdb.AncientReaderOp, id uint64) []byte {
...
// state).
func ReadStateHistoryMetaList(db corexdb.AncientReaderOp, start uint64, count uint64) ([][]byte, error) {
...
func ReadStateAccountIndex(db corexdb.AncientReaderOp, id uint64) []byte {
...
// state history. Compute the position of state history in freezer by minus one
// since the id of first state history starts from one(zero for initial state).
func ReadStateStorageIndex(db corexdb.AncientReaderOp, id uint64) []byte {
...
// state history. Compute the position of state history in freezer by minus one
// since the id of first state history starts from one(zero for initial state).
func ReadStateAccountHistory(db corexdb.AncientReaderOp, id uint64) []byte {
...
// state history. Compute the position of state history in freezer by minus one
// since the id of first state history starts from one(zero for initial state).
func ReadStateStorageHistory(db corexdb.AncientReaderOp, id uint64) []byte {
...
func ReadStateHistory(db corexdb.AncientReaderOp, id uint64) ([]byte, []byte, []byte, []byte, []byte, error) {
...
func WriteStateHistory(db corexdb.AncientWriter, id uint64, meta []byte, accountIndex []byte, storageIndex []byte, accounts []byte, storages []byte) {
	db.ModifyAncients(func(op corexdb.AncientWriteOp) error {
