...
// Package core implements the CoreXChain consensus protocol.
package core
...
type CacheConfig struct {
	...
	TrieDirtyDisabled   bool          // Whether to disable trie write caching and GC altocorexer (archive node)
	TrieTimeLimit       time.Duration // Time limit after which to flush the current in-memory trie to disk
	SnapshotLimit       int           // Memory allowance (MB) to use for caching snapshot entries in memory
	Preimages           bool          // Whether to store preimage of trie key to the disk
	StateHistory        uint64        // Number of blocks from head whose state histories are reserved.
	StateScheme         string        // Scheme used to store corexchain states and merkle tree nodes on top
...
type txLookup struct {
	lookup      *rawdb.DirectTxLookupEntry
...
type BlockChain struct {
	...
	db            corexdb.Database                 // Low level persistent database to store final content in
...
// available in the database. It initialises the default CoreXChain Validator
// and Processor.
func NewBlockChain(db corexdb.Database, cacheConfig *CacheConfig, genesis *Genesis, overrides *ChainOverrides, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(header *types.Header) bool, txLookupLimit *uint64) (*BlockChain, error) {
	if cacheConfig == nil {
	...
	// If CoreX is initialized with an external ancient store, re-initialize the
	...
	// the head block (corexhash cache or clique voting snapshot). Might as well do
	// it in advance.
...
func (bc *BlockChain) SetHead(head uint64) error {
	...
	// Send chain head event to update the flow space
...
func (bc *BlockChain) setHeadBeyondRoot(head uint64, time uint64, root common.Hash, repair bool) (uint64, error) {
	...
	updateFn := func(db corexdb.KeyValueWriter, header *types.Header) (*types.Header, bool) {
	...
	delFn := func(db corexdb.KeyValueWriter, hash common.Hash, num uint64) {
	...
	// touching the header chain altocorexer, unless the freezer is broken
	if repair {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	for i, block := range blockChain {
		...
		// Here we also validate that bundle transactions in the block do not contain an attachment.
		// While the attachment does not affect the block hash / tx hash, sending blobs within a block is not allowed.
		for txIndex, tx := range block.Transactions() {
			if tx.Type() == types.BundleTxType && tx.BundleAttachment() != nil {
				return 0, fmt.Errorf("block #%d contains unexpected blob attachment in tx at index %d", block.NumberU64(), txIndex)
			}
		}
	}
	...
	writeLive := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			if batch.ValueSize() >= corexdb.IdealBatchSize {
				if err := batch.Write(); err != nil {
					return 0, err
				}
...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB) error {
	...
	if nodes > limit || imgs > 4*1024*1024 {
		bc.triedb.Cap(limit - corexdb.IdealBatchSize)
	}
...
// it's not checked in the CoreX side anymore.
...
func (bc *BlockChain) collectLogs(b *types.Block, removed bool) []*types.Log {
	...
	excessBundleResource := b.ExcessBundleResource()
	if excessBundleResource != nil {
		blobGasPrice = eip4844.CalcBlobFee(*excessBundleResource)
	}
