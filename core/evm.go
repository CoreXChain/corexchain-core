...
func NewEVMBlockContext(header *types.Header, chain ChainContext, author *common.Address) vm.BlockContext {
	...
	// If we don't have an explicit author, extract it from the header.
	if author == nil {
	...
	if header.ExcessBundleResource != nil {
		blobBaseFee = eip4844.CalcBlobFee(*header.ExcessBundleResource)
	}
...
func NewEVMTxContext(msg *Message) vm.TxContext {
	ctx := vm.TxContext{
		Origin:       msg.From,
		ValueFlowFee: new(big.Int).Set(msg.ValueFlowFee),
		BundleHashes:   msg.BundleHashes,
	}
	if msg.BundleValueFeeCap != nil {
		ctx.BundleValueFeeCap = new(big.Int).Set(msg.BundleValueFeeCap)
	}
	return ctx
}
